...
class GeneratedForm extends GeneratedCode
{
    protected $_code_arr = [
        'start' => '<?php',
...
//                'function' => '    foreach($data as $column => $value){#        if(isset($this->_fields[$column])){#            if(is_array($this->_fields[$column]["value"])){#                $this->_fields[$column]["value"]["default_value"] = $value;#            }else{#                $this->_fields[$column][\'value\'] = $value;#            }#        }#    }#    return $this;'
                'function' => '    foreach($data as $column => $value){#        if(isset($this->_fields[$column])){#            if($this->_fields[$column][\'type\'] == \'checkbox\' || $this->_fields[$column][\'type\'] == \'select\' || $this->_fields[$column][\'type\'] == \'resource\'){#                $this->_fields[$column][\'value\'][\'default_value\'] = $value;#            }else{#                $this->_fields[$column][\'value\'] = $value;#            }#        }#    }#    return $this;'
            ],
            'getView' => [
        ...
        'protected_function' => [
            'setValue' => [
                ...
                'function' => '    foreach($value_array as $name => $value){#        foreach($this->_fields as $input_name => $value_arr){#            if($input_name == $name){#                if(is_array($this->_fields[$input_name][\'value\'])){                    #$this->_fields[$input_name][\'default_value\'] = $value;                }else{                    $this->_fields[$input_name][\'value\'] = $value;#                }#            }#        }#    }'
            ],
            'setError' => [
