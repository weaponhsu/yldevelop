...
class GeneratedControllers extends GeneratedCode
{
    ...
    protected $_code_arr = [
        ...
        'use' => [
            ...
            'models\Service\#table_name#Service as #table_prefix#Service',
            'models\Transformer\#table_name#Transformer as #table_prefix#Transformer',
                    ...
                    case 'use':
                        foreach($element as $id => $use_namespace){
                            if(strpos($use_namespace, '#table_name#') !== false){
                                ...
                                $this->__table_construct = $this->_getTableConstruct($table_name);
                                ...
                                if(strpos($use_namespace, '#table_prefix#') !== false && $table_name){
                                    $element[$id] = str_replace("#table_prefix#", strtoupper(substr($table_name, 0, 1)), $element[$id]);
                                }
                            }
                        }
...
//                '##        $this->' . ($type == 'protected' ? '_' : ($type == 'private' ? '__' : '')) . strtolower($table_name) . '->setFields($old_field);';
    ...
    public function genDataActionFunctionMain($file_path = ''){
        ...
        if(!empty($file_path)){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
            $code = "    if(Registry::get('method') == 'POST'){#        $" . $table_name . "_service = " . $table_prefix . "Service::getInstance();".
                "#        \$condition = !isset(Registry::get('parameters')['filters']) ? [] : json_decode(Registry::get('parameters')['filters'], true);".
                "#        \$result = \$" . $table_name . "_service->setForeignRelationship(\$this->__foreign_relationship_result)->getList(".
                "#            \$condition,".
                "#            isset(Registry::get(\"parameters\")[\"page\"]) ? Registry::get(\"parameters\")[\"page\"] : 1," .
                "#            isset(Registry::get(\"parameters\")[\"page_size\"]) ? Registry::get(\"parameters\")[\"page_size\"] : Registry::get('config')['admin']['page_size']," .
                "#            isset(Registry::get(\"parameters\")[\"sidx\"]) ? Registry::get(\"parameters\")[\"sidx\"] : \"\"," .
                "#            isset(Registry::get(\"parameters\")[\"sord\"]) ? Registry::get(\"parameters\")[\"sord\"] : \"\"" .
                "#        );".
                "#        \$return = new " . $table_prefix . "Transformer(\$result);".
                "#        return \$this->_responseJson(\$return->BackEndData());".
                "#    }".
                "#    throw new Exception(json_encode(['message' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD, 'title' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD]),'405');";
        }
        return $code;
     ...
     * @return mixed
     */
    public function genDataActionFunctionMainV2($file_path = ''){
        $code = '';
        if(!empty($file_path)){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
//            $code = "    if(Registry::get('method') == 'POST'){#        $" . $table_name . "_service = new " . $table_prefix . "Service();".
            $code = "    if(Registry::get('method') == 'POST'){#        $" . $table_name . "_service = " . $table_prefix . "Service::getInstance();".
                "#        \$condition = !isset(Registry::get('parameters')['filters']) ? [] : json_decode(Registry::get('parameters')['filters'], true);".
                "#        \$result = \$" . $table_name . "_service->setForeignRelationship(\$this->__foreign_relationship_result)->getList(".
                "#            \$condition,".
                "#            isset(Registry::get(\"parameters\")[\"page\"]) ? Registry::get(\"parameters\")[\"page\"] : 1," .
                "#            isset(Registry::get(\"parameters\")[\"page_size\"]) ? Registry::get(\"parameters\")[\"page_size\"] : 10," .
                "#            isset(Registry::get(\"parameters\")[\"sidx\"]) ? Registry::get(\"parameters\")[\"sidx\"] : \"\"," .
                "#            isset(Registry::get(\"parameters\")[\"sord\"]) ? Registry::get(\"parameters\")[\"sord\"] : \"\"" .
                "#        );".
                "#        \$return = new " . $table_prefix . "Transformer(\$result);".
//                "#        echo json_encode(\$return->BackEndData());".
                "#        return \$this->_responseJson(\$return->BackEndData());".
                "#    }".
                "#    throw new Exception(json_encode(['message' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD, 'title' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD]),'405');";
        }
     ...
     * @return mixed
     */
    public function genDataActionFunctionMainV1($file_path = ''){
        $code = '';
        if(!empty($file_path)){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
...
//                "Registry::get('config')['admin']['page_size'], Registry::get('parameters')['sidx'] ? Registry::get('parameters')['sidx'] : '', Registry::get('parameters')['sord'] ? Registry::get('parameters')['sord'] : ''));#   }";
    ...
    public function genStoreActionFunctionMain($file_path = '', $type = 'public')
    {
        ...
        if (!empty($file_path) && in_array($type, ['public', 'protected', 'private'])) {
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/') + 1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
            $code = "    if(Registry::get(\"method\") == \"POST\"){" .
                "#        //表单验证成功" .
                "#        if(\$this->" . ($type == 'protected' ? '_' : ($type == 'private' ? '__' : '')) . $table_name . "_form->isValid() === true){" .
                "#            \$" . $table_name . "_service = " . $table_prefix . "Service::getInstance();" .
                "#            \$" . $table_name . "_service = \$this->setVariable4Service(\$" . $table_name . "_service);" .
                "#            if(\$" . $table_name . "_service instanceof Exception) throw \$" . $table_name . "_service;";
            //20170811 增加相册代码
            if (in_array('album_id', $this->__field_arr)) {
                $code .= "##            //插入相册 " .
                    "#            \$album_str = isset(Registry::get(\"parameters\")['album_id']) ? Registry::get(\"parameters\")['album_id'] : '';" .
                    "#            if (!empty(\$album_str)){" .
                    "#                \$create_album = AlbumTrait::createAlbum(\$album_str);" .
                    "#                if (!\$create_album instanceof Exception){" .
                    "#                   \$" . $table_name . "_service->setAlbumId(\$create_album->__get('_id'));" .
                    "#                }" .
                    "#            }";
            }
            $code .= "#            \$result = \$" . $table_name . "_service->create()->get" . ucfirst($table_name) . "ModelObj();" .
                "#            if(\$result instanceof Exception) throw \$result;" .
                "##            \$parameter_arr = [" .
                "#                \"redirect_url\" => \"/admin/" . (strpos($table_name, '_') === false ? $table_name : implode('', explode('_', $table_name))) . "/index\"," .
     ...
     * @return mixed
     */
    public function genStoreActionFunctionMainV2($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
            $code = "    if(Registry::get(\"method\") == \"POST\"){" .
                "#        //表单验证成功".
                "#        if(\$this->" . ($type == 'protected' ? '_' : ($type == 'private' ? '__' : '')) . $table_name . "_form->isValid() === true){" .
                "#            \$" . $table_name . "_service = new " . $table_prefix . "Service();".
                "#            \$" . $table_name . "_service = \$this->setVariable4Service(\$" . $table_name . "_service);".
                "#            if(\$" . $table_name . "_service instanceof Exception) throw \$" . $table_name . "_service;".
                "#            \$result = \$" . $table_name . "_service->create()->get" . ucfirst($table_name) . "ModelObj();".
                "#            if(\$result instanceof Exception) throw \$result;".
                "##            \$parameter_arr = [".
                "#                \"redirect_url\" => \"/admin/" . (strpos($table_name, '_') === false ? $table_name : implode('', explode('_', $table_name))) . "/index\"," .
                "#                \"title\" => ErrorTips::" . strtoupper($table_name) . "_CREATE_SUCCESS," .
...
//        return str_replace('%', '$', $code);
     ...
     * @return mixed
     */
    public function genStoreActionFunctionMainV1($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
...
//                "                %" . $table_name . "_service = new " . ucfirst($table_name) . "Service();#                %this->__refreshFields(%". $table_name ."_service);##";
                "                %" . $table_name . "_service = " . ucfirst($table_name) . "Service::getInstance();#                %this->__refreshFields(%". $table_name ."_service);##";
    ...
    public function genUpdateActionFunctionMain($file_path = '', $type = 'public')
    {
        ...
        if (!empty($file_path) && in_array($type, ['public', 'protected', 'private'])) {
            ...
            $code = "    if(Registry::get(\"method\") == \"POST\"){" .
                "#        if(\$this->" . ($type == 'protected' ? '_' : ($type == 'private' ? '__' : '')) . $table_name . "_form->isValid() === true){" .
                "#            \$" . $table_name . "_service = " . $table_prefix . "Service::getInstance();" .
                "#            \$" . $table_name . "_service = \$this->setVariable4Service(\$" . $table_name . "_service, 'update');" .
                "#            if(\$" . $table_name . "_service instanceof Exception) throw \$" . $table_name . "_service;";
            //20170811 增加相册代码
            if (in_array('album_id', $this->__field_arr)) {
                $code .= "##            //插入相册 " .
                    "#            \$album_id = isset(Registry::get(\"parameters\")['default_value']) ? Registry::get(\"parameters\")['default_value'] : 0;" .
                    "#            \$album_str = isset(Registry::get(\"parameters\")['album_id']) ? Registry::get(\"parameters\")['album_id'] : '';" .
                    "#            \$update_album = AlbumTrait::updateAlbum(\$album_str, \$album_id);" .
                    "#            if (!\$update_album instanceof Exception){" .
                    "#                \$" . $table_name . "_service->setAlbumId(\$update_album->__get('_id'));" .
                    "#            }";
            }
            $code .= "#            \$result = \$" . $table_name . "_service->update()->get" . ucfirst($table_name) . "ModelObj();" .
                "#            if(\$result instanceof Exception) throw \$result;" .
                "##            \$parameter_arr = [" .
                ...
                "#             \$this->_setHeader();" .
                "#             \$this->_setLeftMenu();" .
                "#             \$this->getView()->assign('form',\$this->" . ($type == 'protected' ? '_' : ($type == 'private' ? '__' : '')) . $table_name . "_form->generateForm('edit', 'update', 'post')->getView());#" .
                "#             \$this->display('edit');" .
                "#        }" .
                "#    }" .
                "#    throw new Exception(json_encode(['message' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD, 'title' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD]),'405');";
     ...
     * @return mixed
     */
    public function genUpdateActionFunctionMainV2($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
            ...
            $code = "    if(Registry::get(\"method\") == \"POST\"){" .
                ...
                "#            \$". $table_name ."_service = \$this->setVariable4Service(\$" . $table_name . "_service, \"update\");".
                "#            if(\$" . $table_name . "_service instanceof Exception) throw \$" . $table_name . "_service;".
                "#            \$result = \$" . $table_name . "_service->update()->get" . ucfirst($table_name) . "ModelObj();" .
                "#            if(\$result instanceof Exception) throw \$result;".
                "##            \$parameter_arr = [".
                "#                 \"redirect_url\" => \"/admin/" . $table_name . "/index\",".
                "#                 \"title\" => ErrorTips::" . strtoupper($table_name) . "_EDIT_SUCCESS,".
     ...
     * @return mixed
     */
    public function genUpdateV2ActionFunctionMain($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            /*$table_name_arr = [];
    ...
    public function genDeleteActionFunctionMain($file_path = '', $type = 'public'){
        ...
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])) {
            ...
            $code = "    if(Registry::get(\"method\") == \"POST\"){" .
                ...
                "#        if(!empty(\$id_arr)){" .
                "#            \$result = count(\$id_arr) == 1 ?" .
                "#                " . $table_prefix . "Service::getInstance()->setId(\$id_arr[0])->delete() :" .
                "#                " . $table_prefix . "Service::getInstance()->batchDelete([\"id\" => \$id_arr]);" .
                "#            if(\$result instanceof Exception)" .
                "#                \$return_arr = [\"msgCode\" => \$result->getCode(), \"message\" => \$result->getMessage()];" .
                "#            else" .
                "#                \$return_arr = [\"msgCode\" => '000', \"message\" => ErrorTips::" . strtoupper($table_name) . "_DELETE_SUCCESS];" .
                "#            return \$this->_responseJson(\$return_arr);" .
                "#        }" .
                "#        throw new Exception(json_encode(['message' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD, 'title' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD]), '200');" .
                "#    }" .
                "#    throw new Exception(json_encode(['message' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD, 'title' => ErrorMsg::CONTROLLER_INVALID_REQUEST_METHOD]), '405');";
        }
        return $code;
//        return str_replace('%', '$', $code);
     ...
     * @return mixed
     */
    public function genDeleteActionFunctionMainV2($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
            ...
            $code = "#    \$return_arr = ['msgCode' => '002', 'message' => ErrorTips::" . strtoupper($table_name) . "_NOT_EXISTS];" .
                ...
                "#        if(!empty(\$id_arr)){" .
                "#            \$" . $table_name . "_service = new " . $table_prefix . "Service();".
                "#            \$result = count(\$id_arr) == 1 ? \$" . $table_name . "_service->setId(\$id_arr[0])->delete() : \$" . $table_name . "_service->batchDelete([\"id\" => \$id_arr]);" .
                "#            if(\$result instanceof Exception)".
                "#                \$return_arr = [\"msgCode\" => \$result->getCode(), \"message\" => \$result->getMessage()];".
                "#            else".
                "#                \$return_arr = [\"msgCode\" => '000', \"message\" => ErrorTips::" . strtoupper($table_name) . "_DELETE_SUCCESS];" .
     ...
     * @return mixed
     */
    public function genDeleteActionFunctionMainV1($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            $table_prefix = strtoupper(substr($table_name, 0, 1));
...
//        return str_replace('%', '$', $code);
     ...
     * @return mixed
     */
    public function genDeleteV2ActionFunctionMain($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) && in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            /*$table_name_arr = [];
...
//            $code = "    \$" . $table_name . "_service = new " . $table_prefix . "service();".
            $code = "    \$" . $table_name . "_service = " . $table_prefix . "service::getInstance();".
                "#    \$result = \$" . $table_name . "_service->getOne(Registry::get(\"parameters\")[\"id\"])->get" . ucfirst($table_name) . "ModelObj();".
                "#    if(\$result instanceof Exception)".
                "#        throw \$result;".
                "##    \$transformer = new " . $table_prefix . "Transformer(\$result);".
                "#    \$" . $table_name . "_is_exists[\"data\"] = \$transformer->SingleData();".
                "##    \$this->_setHeader();".
                "#    \$this->_setLeftMenu();".
                "#    \$this->getView()->assign('form', \$this->" . ($type == 'protected' ? '_' : ($type == 'private' ? '__' : '')) . $table_name . "_form->generateForm('edit', 'update', 'post')->fillData(\$" . $table_name . "_is_exists['data'])->getView());" .
                "#    \$this->display(\"edit\");";
        }
        return $code;
//        return str_replace('%', '$', $code);
     ...
     * @return mixed
     */
    public function genEditActionFunctionMainV1($file_path = '', $type = 'public'){
        $code = '';
        if(!empty($file_path) || in_array($type, ['public', 'protected', 'private'])){
            $table_name = strtolower(substr($file_path, strrpos($file_path, '/')+1, (strrpos($file_path, '.') - 1 - strrpos($file_path, '/'))));
            /*$table_name_arr = [];
