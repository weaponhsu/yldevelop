...
use models\Transformer\MenuTransformer;;
...
class MenuService {
     ...
     * 列表缓存sortedSet
     */
    private $__list_sorted_set_cache_name_prefix = 'menu:list';

    /**
     * 单条数据缓存string
     */
    private $__single_string_cache_name_prefix = 'admin:menu';
    ...
    public function getOne($condition = [], $cache = false, $cache_name = "", $expire = 86400) {
        ...
        if($select_db === true){
            ...
            $data_from_db = is_array($condition) ? MenuModel::getInstance()->findOneBy($condition) :
                MenuModel::getInstance()->find($condition);

            $is_into_redis = $cache;

            $menu_transformer = new MenuTransformer($data_from_db);
            $redis_is_exists = $menu_transformer->SingleData();
        }
    ...
    public function getList($page = 1, $page_size = 20, $sort = "", $order = "", $condition = [], $cache = false, $cache_name = "", $expire = 86400) {
        ...
        // 读取数据库数据
        if ($select_db === true)
            $result_from_db = MenuModel::getInstance()->__set('page', $page)->__set('page_size', $page_size)
                ->setOrderBy(['order' => $order, 'sort' => $sort])->findBy($condition);
        // 数据库查无结果 或 select_db === false
        // 从数据库中查询出结果了
        if ($result_from_db instanceof MenuModel) {
            $transformer = new menuTransformer($result_from_db);
            // 根据是否使用缓存来获取数据
     ...
     * @param string $url 特殊URL
     * @param int $display 是否显示到左侧菜单(0为否,1为是,默认0)
     * @param int $is_operation 是否是操作(0为否,1为是,默认0)
     * @param int $list_order 排序
     * @param int $created_by 创建人
     * @param int $updated_by 最后一次编辑人
     * @param bool $cache 是否删除缓存
     * @param string $cache_name 缓存名称
     * @param int $expire redis有效期
     * @return array
     ...
     * @throws TransformerException
     */
    public function create($name = "", $parent = 0, $path = "", $url = "", $display = 0, $is_operation = 0, $list_order = 0, $created_by = 0, $updated_by = 0, $cache = false, $cache_name = "", $expire = 86400) {
        $is_into_redis = $delete_list_sorted_set = false;

        $menu_model = MenuModel::getInstance();
        if (! method_exists($menu_model, '__set'))
            throw new ModelReflectionException(ModelException::THERE_IS_NO_SET_METHOD, ModelException::THERE_IS_NO_SET_METHOD_NO);
        if (! property_exists($menu_model, 'primary_key_arr'))
        ...
        $primary_key = null;
        foreach ($menu_model->primary_key_arr as $primary_key) {
            if (property_exists($menu_model, 'auto_increment_key_arr') && in_array($primary_key, $menu_model->auto_increment_key_arr))
                $menu_model->__set($primary_key, null);
        }

        // 插入数据库
        $new_menu_primary_key = $menu_model
            ->__set('name', $name)
            ->__set('parent', $parent)
            ->__set('path', $path)
            ->__set('url', $url)
            ->__set('display', $display)
            ->__set('is_operation', $is_operation)
            ->__set('list_order', $list_order)
            ->__set('created_by', $created_by)
            ->__set('updated_by', $updated_by)
            ->insert();

        if (! is_null($primary_key))
            $menu_model->__set($primary_key, $new_menu_primary_key);

        // 插入成功，生成创建时间，更新时间，登录次数 生成返回用数据
        $datetime_now = date('Y-m-d H:i:s', time());
        $menu_model->__set('created_at', $datetime_now);
        $menu_model->__set('updated_at', $datetime_now);
        $transformer = new MenuTransformer(MenuModel::getInstance());
        $data_from_storage = $transformer->SingleData();

        // 需要使用缓存 is_into_redis为true delete_list_sorted_set为true
        if($cache === true){
            $cache_name = $this->__single_string_cache_name_prefix . ':' . $new_menu_primary_key;
    ...
    public function update($update_column = [], $primary_key = "", $cache = false, $cache_name = "", $expire = 86400) {
        ...
        foreach ($update_column as $column_name => $value) {
            $menu_model->__set($column_name, $value);
            if ($update_redis === true)
                $redis_is_exists[$column_name] = $value;
        }
        // 修改数据库
        $menu_model->__set(implode('', $menu_model->primary_key_arr), $primary_key)->update();

        // 不需要使用缓存 或缓存不存在时 读取数据库
        // 之所以再从数据库中获取一边，是为了返回给调用者一个menu
        if ($select_db === true) {
            // 直接使用主键查询
            $data_from_db = $menu_model->find($primary_key);
    ...
    public function delete($primary_key = "", $cache = false, $cache_name = "", $expire = 86400) {
        ...
        if ($delete_from_db === true)
            // 删除数据库
            MenuModel::getInstance()->__set(MenuModel::getInstance()->primary_key_arr[0], $primary_key)->delete();
    ...
    public function batchInsert($arr = []) {
        ...
        $menu_model->__set('data', $arr)->batchInsert();
        // 获取列表缓存名
    ...
    public function batchUpdate($update_column = [], $cache = false, $expire = 86400) {
        ...
        $menu_model = MenuModel::getInstance();
        // 更新数据库
        $menu_model->__set('data', $update_column)->genBatchUpdateSql();
        ...
        // 缓存不存在 或不需要读取缓存
            ...
            $redis_is_exists = $this->getList(1, count($update_column), 'desc', $menu_model->primary_key_arr[0], [
                'groupOp' => 'AND', 'rules' => [
                    ['field' => $menu_model->primary_key_arr[0], 'op' => 'in', 'data' => implode(',', array_column($update_column, $menu_model->primary_key_arr[0]))]]
            ]);
            $into_redis = $cache;
        ...
        if ($redis_is_exists && $cache === true) {
            $match_update_column = array_column($update_column, $menu_model->primary_key_arr[0]);
            foreach ($redis_is_exists as $idx => $record) {
                if (false !== $key = array_search($record[$menu_model->primary_key_arr[0]], $match_update_column)) {
                    foreach ($update_column[$key] as $column => $new_value)
                        ...
                        if ($into_redis === true)
                            array_push($cache_name_arr, $this->__single_string_cache_name_prefix . ':' . $record[$menu_model->primary_key_arr[0]]);
                }
            }
        }
    ...
    public function batchDelete($arr = [], $cache = false) {
        ...
        $menu_model = MenuModel::getInstance();
        $menu_model->__set("data", $array)->batchDelete();
