...
use models\Transformer\AuthorityTransformer;;
...
class AuthorityService {
     ...
     * 列表缓存sortedSet
     */
    private $__list_sorted_set_cache_name_prefix = 'authority:list';

    /**
     * 单条数据缓存string
     */
    private $__single_string_cache_name_prefix = 'admin:authority';
    ...
    public function getOne($condition = [], $cache = false, $cache_name = "", $expire = 86400) {
        ...
        if($select_db === true){
            ...
            $data_from_db = is_array($condition) ? AuthorityModel::getInstance()->findOneBy($condition) :
                AuthorityModel::getInstance()->find($condition);

            $is_into_redis = $cache;

            $authority_transformer = new AuthorityTransformer($data_from_db);
            $redis_is_exists = $authority_transformer->SingleData();
        }
    ...
    public function getList($page = 1, $page_size = 20, $sort = "", $order = "", $condition = [], $cache = false, $cache_name = "", $expire = 86400) {
        ...
        // 读取数据库数据
        if ($select_db === true)
            $result_from_db = AuthorityModel::getInstance()->__set('page', $page)->__set('page_size', $page_size)
                ->setOrderBy(['order' => $order, 'sort' => $sort])->findBy($condition);
        // 数据库查无结果 或 select_db === false
        // 从数据库中查询出结果了
        if ($result_from_db instanceof AuthorityModel) {
            $transformer = new authorityTransformer($result_from_db);
            // 根据是否使用缓存来获取数据
     ...
     * @throws TransformerException
     */
    public function create($rule_id = 0, $role_id = 0, $cache = false, $cache_name = "", $expire = 86400) {
        $is_into_redis = $delete_list_sorted_set = false;

        $authority_model = AuthorityModel::getInstance();
        if (! method_exists($authority_model, '__set'))
            throw new ModelReflectionException(ModelException::THERE_IS_NO_SET_METHOD, ModelException::THERE_IS_NO_SET_METHOD_NO);
        if (! property_exists($authority_model, 'primary_key_arr'))
        ...
        $primary_key = null;
        foreach ($authority_model->primary_key_arr as $primary_key) {
            if (property_exists($authority_model, 'auto_increment_key_arr') && in_array($primary_key, $authority_model->auto_increment_key_arr))
                $authority_model->__set($primary_key, null);
        }

        // 插入数据库
        $new_authority_primary_key = $authority_model
            ->__set('rule_id', $rule_id)
            ->__set('role_id', $role_id)
            ->insert();

        if (! is_null($primary_key))
            $authority_model->__set($primary_key, $new_authority_primary_key);
        $transformer = new AuthorityTransformer(AuthorityModel::getInstance());
        $data_from_storage = $transformer->SingleData();

        // 需要使用缓存 is_into_redis为true delete_list_sorted_set为true
        if($cache === true){
            $cache_name = $this->__single_string_cache_name_prefix . ':' . $new_authority_primary_key;
    ...
    public function update($update_column = [], $primary_key = "", $cache = false, $cache_name = "", $expire = 86400) {
        ...
        foreach ($update_column as $column_name => $value) {
            $authority_model->__set($column_name, $value);
            if ($update_redis === true)
                $redis_is_exists[$column_name] = $value;
        }
        // 修改数据库
        $authority_model->__set(implode('', $authority_model->primary_key_arr), $primary_key)->update();

        // 不需要使用缓存 或缓存不存在时 读取数据库
        // 之所以再从数据库中获取一边，是为了返回给调用者一个authority
        if ($select_db === true) {
            // 直接使用主键查询
            $data_from_db = $authority_model->find($primary_key);
    ...
    public function delete($primary_key = "", $cache = false, $cache_name = "", $expire = 86400) {
        ...
        if ($delete_from_db === true)
            // 删除数据库
            AuthorityModel::getInstance()->__set(AuthorityModel::getInstance()->primary_key_arr[0], $primary_key)->delete();
    ...
    public function batchInsert($arr = []) {
        ...
        $authority_model->__set('data', $arr)->batchInsert();
        // 获取列表缓存名
    ...
    public function batchUpdate($update_column = [], $cache = false, $expire = 86400) {
        ...
        $authority_model = AuthorityModel::getInstance();
        // 更新数据库
        $authority_model->__set('data', $update_column)->genBatchUpdateSql();
        ...
        // 缓存不存在 或不需要读取缓存
            ...
            $redis_is_exists = $this->getList(1, count($update_column), 'desc', $authority_model->primary_key_arr[0], [
                'groupOp' => 'AND', 'rules' => [
                    ['field' => $authority_model->primary_key_arr[0], 'op' => 'in', 'data' => implode(',', array_column($update_column, $authority_model->primary_key_arr[0]))]]
            ]);
            $into_redis = $cache;
        ...
        if ($redis_is_exists && $cache === true) {
            $match_update_column = array_column($update_column, $authority_model->primary_key_arr[0]);
            foreach ($redis_is_exists as $idx => $record) {
                if (false !== $key = array_search($record[$authority_model->primary_key_arr[0]], $match_update_column)) {
                    foreach ($update_column[$key] as $column => $new_value)
                        ...
                        if ($into_redis === true)
                            array_push($cache_name_arr, $this->__single_string_cache_name_prefix . ':' . $record[$authority_model->primary_key_arr[0]]);
                }
            }
        }
    ...
    public function batchDelete($arr = [], $cache = false) {
        ...
        $authority_model = AuthorityModel::getInstance();
        $authority_model->__set("data", $array)->batchDelete();
