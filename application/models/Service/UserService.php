...
use models\Transformer\UserTransformer;;
...
class UserService {
     ...
     * 列表缓存sortedSet
     */
    private $__list_sorted_set_cache_name_prefix = 'user:list';

    /**
     * 单条数据缓存string
     */
    private $__single_string_cache_name_prefix = 'admin:user';
    ...
    public function getOne($condition = [], $cache = false, $cache_name = "", $expire = 86400) {
        ...
        if($select_db === true){
            ...
            $data_from_db = is_array($condition) ? UserModel::getInstance()->findOneBy($condition) :
                UserModel::getInstance()->find($condition);

            $is_into_redis = $cache;

            $user_transformer = new UserTransformer($data_from_db);
            $redis_is_exists = $user_transformer->SingleData();
        }
    ...
    public function getList($page = 1, $page_size = 20, $sort = "", $order = "", $condition = [], $cache = false, $cache_name = "", $expire = 86400) {
        ...
        // 读取数据库数据
        if ($select_db === true)
            $result_from_db = UserModel::getInstance()->__set('page', $page)->__set('page_size', $page_size)
                ->setOrderBy(['order' => $order, 'sort' => $sort])->findBy($condition);
        // 数据库查无结果 或 select_db === false
        // 从数据库中查询出结果了
        if ($result_from_db instanceof UserModel) {
            $transformer = new userTransformer($result_from_db);
            // 根据是否使用缓存来获取数据
     ...
     * @param string $google_secret 谷歌认证器
     * @param int $role 角色
     * @param string $mobile 手机号码
     * @param int $stats 状态(0为封禁,1为允许登录,默认0)
     * @param string $uuid uuid
     * @param int $login_times 登录次数
     * @param int $created_by 创建人
     * @param int $updated_by 最后一次修改人
     * @param bool $cache 是否删除缓存
     * @param string $cache_name 缓存名称
     * @param int $expire redis有效期
     * @return array
     ...
     * @throws TransformerException
     */
    public function create($username = "", $password = "", $google_secret = "", $role = 0, $mobile = "", $stats = 0, $uuid = "", $login_times = 0, $created_by = 0, $updated_by = 0, $cache = false, $cache_name = "", $expire = 86400) {
        $is_into_redis = $delete_list_sorted_set = false;

        $user_model = UserModel::getInstance();
        if (! method_exists($user_model, '__set'))
            throw new ModelReflectionException(ModelException::THERE_IS_NO_SET_METHOD, ModelException::THERE_IS_NO_SET_METHOD_NO);
        if (! property_exists($user_model, 'primary_key_arr'))
        ...
        $primary_key = null;
        foreach ($user_model->primary_key_arr as $primary_key) {
            if (property_exists($user_model, 'auto_increment_key_arr') && in_array($primary_key, $user_model->auto_increment_key_arr))
                $user_model->__set($primary_key, null);
        }

        // 插入数据库
        $new_user_primary_key = $user_model
            ->__set('username', $username)
            ->__set('password', $password)
            ->__set('google_secret', $google_secret)
            ->__set('role', $role)
            ->__set('mobile', $mobile)
            ->__set('stats', $stats)
            ->__set('uuid', $uuid)
            ->__set('login_times', $login_times)
            ->__set('created_by', $created_by)
            ->__set('updated_by', $updated_by)
            ->insert();

        if (! is_null($primary_key))
            $user_model->__set($primary_key, $new_user_primary_key);

        // 插入成功，生成创建时间，更新时间，登录次数 生成返回用数据
        $datetime_now = date('Y-m-d H:i:s', time());
        $user_model->__set('last_login_time', $datetime_now);
        $user_model->__set('created_at', $datetime_now);
        $user_model->__set('updated_at', $datetime_now);
        $transformer = new UserTransformer(UserModel::getInstance());
        $data_from_storage = $transformer->SingleData();

        // 需要使用缓存 is_into_redis为true delete_list_sorted_set为true
        if($cache === true){
            $cache_name = $this->__single_string_cache_name_prefix . ':' . $new_user_primary_key;
    ...
    public function update($update_column = [], $primary_key = "", $cache = false, $cache_name = "", $expire = 86400) {
        ...
        foreach ($update_column as $column_name => $value) {
            $user_model->__set($column_name, $value);
            if ($update_redis === true)
                $redis_is_exists[$column_name] = $value;
        }
        // 修改数据库
        $user_model->__set(implode('', $user_model->primary_key_arr), $primary_key)->update();

        // 不需要使用缓存 或缓存不存在时 读取数据库
        // 之所以再从数据库中获取一边，是为了返回给调用者一个user
        if ($select_db === true) {
            // 直接使用主键查询
            $data_from_db = $user_model->find($primary_key);
    ...
    public function delete($primary_key = "", $cache = false, $cache_name = "", $expire = 86400) {
        ...
        if ($delete_from_db === true)
            // 删除数据库
            UserModel::getInstance()->__set(UserModel::getInstance()->primary_key_arr[0], $primary_key)->delete();
    ...
    public function batchInsert($arr = []) {
        ...
        $user_model->__set('data', $arr)->batchInsert();
        // 获取列表缓存名
    ...
    public function batchUpdate($update_column = [], $cache = false, $expire = 86400) {
        ...
        $user_model = UserModel::getInstance();
        // 更新数据库
        $user_model->__set('data', $update_column)->genBatchUpdateSql();
        ...
        // 缓存不存在 或不需要读取缓存
            ...
            $redis_is_exists = $this->getList(1, count($update_column), 'desc', $user_model->primary_key_arr[0], [
                'groupOp' => 'AND', 'rules' => [
                    ['field' => $user_model->primary_key_arr[0], 'op' => 'in', 'data' => implode(',', array_column($update_column, $user_model->primary_key_arr[0]))]]
            ]);
            $into_redis = $cache;
        ...
        if ($redis_is_exists && $cache === true) {
            $match_update_column = array_column($update_column, $user_model->primary_key_arr[0]);
            foreach ($redis_is_exists as $idx => $record) {
                if (false !== $key = array_search($record[$user_model->primary_key_arr[0]], $match_update_column)) {
                    foreach ($update_column[$key] as $column => $new_value)
                        ...
                        if ($into_redis === true)
                            array_push($cache_name_arr, $this->__single_string_cache_name_prefix . ':' . $record[$user_model->primary_key_arr[0]]);
                }
            }
        }
    ...
    public function batchDelete($arr = [], $cache = false) {
        ...
        $user_model = UserModel::getInstance();
        $user_model->__set("data", $array)->batchDelete();
